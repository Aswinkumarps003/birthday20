...
// Define the custom colors used in the Tailwind classes for clarity
const customColors = {
    vibBlue: 'rgba(78, 205, 196, 1)',
    vibPink: 'rgba(255, 107, 107, 1)',
    vibPurple: 'rgba(173, 142, 230, 1)',
    vibYellow: 'rgba(255, 230, 109, 1)',
    darkRed: 'rgba(122, 31, 40, 1)',
};
...
export const LettersPage: React.FC = () => {
    ...
    const [showMusic, setShowMusic] = useState(true); // Keep your intended state
    const audioRef = useRef<HTMLAudioElement>(null);
    const hasPlayed = useRef(false); // To track if music has started
    ...
    // --- MUSIC LOGIC (Auto-play on page load) ---

    // Auto-play music when component mounts (user interaction from LandingPage button satisfies autoplay policy)
    useEffect(() => {
        const audio = audioRef.current;
        if (audio && showMusic && !hasPlayed.current) {
            audio.volume = 0.2;
            // Try to play immediately - the click from LandingPage button should allow this
            audio.play()
                .then(() => {
                    hasPlayed.current = true;
                    console.log("Music playback started automatically.");
                })
                .catch(e => {
                    console.log("Auto-play prevented, will try on next user interaction:", e);
                    // If autoplay fails, try on first click/keydown
                    const attemptPlay = () => {
                        if (audio && !hasPlayed.current) {
                            audio.play()
                                .then(() => {
                                    hasPlayed.current = true;
                                    console.log("Music playback started on user interaction.");
                                    window.removeEventListener('click', attemptPlay);
                                    window.removeEventListener('keydown', attemptPlay);
                                })
                                .catch(err => console.log("Still unable to play:", err));
                        }
                    };
    ...
    // Handle pause when showMusic is toggled to false
    useEffect(() => {
        if (audioRef.current) {
            audioRef.current.volume = 0.2;
            if (!showMusic && hasPlayed.current) {
                audioRef.current.pause();
            } else if (showMusic && hasPlayed.current) {
    ...
    return (
        ...
        >
            ...
            <motion.div 
                ...
                style={{
                    background: `linear-gradient(135deg, ${customColors.vibBlue}33 0%, rgba(253,251,247,0) 50%, ${customColors.vibPink}26 100%)`,
                    filter: 'blur(60px)',
            ...
            <motion.div 
                className={`absolute -top-[20%] -left-[10%] w-[80vw] h-[80vw] bg-[${customColors.vibBlue}]/20 rounded-full blur-[100px] pointer-events-none`}
                animate={{ 
                    scale: [1, 1.2, 0.9, 1], 
                    rotate: [0, 20, -10, 0],
                    x: [0, 50, -30, 0],
            ...
            />
            <motion.div 
                className={`absolute top-[30%] -right-[20%] w-[70vw] h-[70vw] bg-[${customColors.vibPurple}]/20 rounded-full blur-[120px] pointer-events-none`}
                animate={{ 
                    scale: [1, 1.3, 0.8, 1], 
                    rotate: [0, -15, 10, 0],
                    x: [0, -60, 40, 0],
            ...
            />
            <motion.div 
                className={`absolute -bottom-[20%] left-[20%] w-[60vw] h-[60vw] bg-[${customColors.vibYellow}]/20 rounded-full blur-[100px] pointer-events-none`}
                animate={{ 
                    scale: [0.9, 1.1, 1], 
                    rotate: [0, 10, -10, 0],
                    x: [0, 30, -30, 0],
            ...
            >
                <svg width="50" height="60" viewBox="0 0 100 120">
                    <path d="M50 95 C 20 95 0 70 0 45 C 0 20 22 0 50 0 C 78 0 100 20 100 45 C 100 70 80 95 50 95 Z" fill={customColors.vibBlue} />
            ...
            >
                <svg width="70" height="85" viewBox="0 0 100 120">
                    <path d="M50 95 C 20 95 0 70 0 45 C 0 20 22 0 50 0 C 78 0 100 20 100 45 C 100 70 80 95 50 95 Z" fill={customColors.vibPink} />
                    <path d="M50 95 Q 50 110 55 120" stroke="#000" strokeWidth="1" fill="none" />
                </svg>
            </motion.div>
            ...
            <audio ref={audioRef} loop src="/violin.mp3" />
            ...
            <div className="relative w-full h-full max-w-6xl px-4 flex-1 flex items-center justify-center perspective-1000 py-4">
                ...
                <AnimatePresence mode="wait" initial={false}>
                    ...
                    >
                        ...
                        {/* Washi Tape - Vibrant */}
                        <div className={`absolute -top-3 left-1/2 transform -translate-x-1/2 w-32 h-8 bg-[${customColors.vibPink}]/70 rotate-[-1deg] shadow-sm backdrop-blur-[1px] z-40 opacity-90`}></div>

                        <div className="relative z-10 flex flex-col items-center w-full h-full" style={{ transform: "translateZ(10px)" }}>
                            
                            {/* Header */}
                            <h2 className="text-3xl md:text-4xl font-serif font-bold text-darkRed mb-6 mt-4 tracking-wide shadow-sm" style={{ color: customColors.darkRed }}>
