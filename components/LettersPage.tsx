...
const MiniBalloon: React.FC<MiniBalloonProps> = ({ index, isActive, onClick, color }) => (
    <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        onClick={onClick}
        className="relative group flex items-center justify-center my-1 mx-1 w-10 h-12 focus:outline-none"
    ...
    >
        ...
        <span className="absolute top-2 text-[10px] font-bold text-white drop-shadow-md pointer-events-none">
            {index + 1}
        </span>
    </motion.button>
);
...
export const LettersPage: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Left Sidebar Navigation (Balloon Panel) */}
      {!showFinalScreen && (
          <motion.div 
      ...
      <div className="relative w-full h-full max-w-6xl px-4 flex-1 flex items-center justify-center perspective-1000 py-4">
        ...
        {showFinalScreen ? (
            ...
            <>
                ...
                {/* Moved slightly to right to accommodate sidebar */}
